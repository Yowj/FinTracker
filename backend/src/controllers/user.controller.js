...
export const addAccount = async (req, res) => {
  try {
    ...
    if (!user || !name || initialBalance === undefined) {
      return res.status(400).json({
        success: false,
        message: "User ID, name, icon and balance are required",
      });
    }
    ...
    user.accounts.push({ name, initialBalance, icon });
    await user.updateAccountBalance();
...
export const deleteAccount = async (req, res) => {
  try {
    ...
    // Delete all transactions that have a category matching the accountName
